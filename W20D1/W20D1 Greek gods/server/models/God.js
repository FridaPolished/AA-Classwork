...
GodSchema.statics.addGodEmblem = (godId, emblemId) => {
  const God = mongoose.model("god");
...
GodSchema.statics.removeGodEmblem = (godId, emblemId) => {
  const God = mongoose.model("god");
  const Emblem = mongoose.model("emblem");
...
GodSchema.statics.updateGodAbode = (godId, abodeId) => {
  const God = mongoose.model("god");
  ...
  return God.findById(godId)
    .populate('abode')
    .then(god => {
      return Abode.findById(abodeId)
        .then(abode => {
          const oldAbode = god.abode;
          if (oldAbode) { oldAbode.gods.pull(godId) }
          god.abode = abodeId;
          abode.gods.push(godId);

          return Promise.all([god.save(), abode.save(), oldAbode ? oldAbode.save() : Promise.resolve()]).then(
            ([god, abode, oldAbode]) => god
          );
        });
    });
};
