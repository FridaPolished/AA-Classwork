...
//     if(nums.length === 0) return 0; //without this the table would have an undefined
//     //asssigned in line 72.
//     let table = new Array(nums.length).fill(0);
//     table[0] = nums[0]; 

//     for(let i = 1; i < table.length; i ++){
//         let left2 = table[i - 2 ] === undefined ? 0 : table[ i - 2];
//         let sum = left2 + nums[i];
//         let left1 = table[i - 1];
//         table[i] = Math.max(sum, left1);
//     }
...
function maxNonAdjacentSum(nums, memo={}) {
    ...
    memo[nums.length] =  Math.max(
        initial + maxNonAdjacentSum(nums.slice(2), memo),
...
// this solution works only for a  small input and gets stucked with larger ones.
...
//         let coin = coins[i];
//         if (coin <= amount){
//             subResult.push(minChange(coins, amount- coin) + 1);
//         }
//        table[i] = Math.min(...subResult);
//     }
...
function minChange(coins, amount){
    ...
    table[0] = 0;
    coins.forEach(coin => {
        for(let cents = 0; cents < table.length; cents ++){
            for(let numCoins = 0; numCoins * coin <= cents; numCoins ++){
               let remanider = cents - numCoins * coin;
               let att = table[remanider] + numCoins;
                if (att < table[cents]) table[cents] = att; 
            }
        }
    });
    return table[table.length-1];
